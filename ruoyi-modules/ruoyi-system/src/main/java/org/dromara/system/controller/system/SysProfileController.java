...
public class SysProfileController extends BaseController {
    ...
    @RepeatSubmit
    @SaCheckLogin
    @Log(title = "重置邀请码", businessType = BusinessType.UPDATE)
    @PostMapping("/uploadInviteCodeInput")
    @Transactional
    public R<Void> uploadInviteCodeInput() {

        SysUserBo sysUser = new SysUserBo();
        sysUser.setUserId(LoginHelper.getUserId());
        sysUser.setInvitationCode( RandomStringUtils.randomAlphanumeric(8));
        for (int i = 0; i < 1000; i++) {
            QueryWrapper<SysUser> sysUserQueryWrapper = new QueryWrapper<>();
            sysUserQueryWrapper.lambda().eq(SysUser::getInvitationCode,sysUser.getInvitationCode());
            Long l = userService.getSysUserMapper().selectCount(sysUserQueryWrapper);
            if (l > 0) {
                sysUser.setInvitationCode( RandomStringUtils.randomAlphanumeric(8));
            }else{
                userService.updateUser(sysUser);
                return R.ok(sysUser.getInvitationCode());
            }
        }
        return R.fail("修改密码异常，请联系管理员");
    }
