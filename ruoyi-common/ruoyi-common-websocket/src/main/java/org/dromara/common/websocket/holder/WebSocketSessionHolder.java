...
public class WebSocketSessionHolder {
    ...
    private static final Map<Long, ConcurrentHashSet<WebSocketSession>> USER_SESSION_MAP = new ConcurrentHashMap<>();
...
//        removeSession(sessionKey);
        USER_SESSION_MAP.computeIfAbsent(sessionKey, k -> new ConcurrentHashSet<>()).add(session);
//        USER_SESSION_MAP.computeIfAbsent(sessionKey, session);
    }
    ...
    public static void removeSession(Long sessionKey) {
        ConcurrentHashSet<WebSocketSession> remove = USER_SESSION_MAP.remove(sessionKey);
        try {
            for (WebSocketSession webSocketSession : remove) {
     ...
     */
    public static ConcurrentHashSet<WebSocketSession> getSessions(Long sessionKey) {
        return USER_SESSION_MAP.getOrDefault(sessionKey,new ConcurrentHashSet<>());
    }
