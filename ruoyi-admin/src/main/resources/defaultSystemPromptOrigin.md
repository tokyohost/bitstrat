...
## Market Parameters
- **Exchange**: #{exchange} (decentralized perpetual futures)
- **Asset Universe**: #{assetUniverse} (perpetual contracts)
- **Starting Capital**: $#{startCapital} USDT
- **Market Hours**: 24/7 continuous trading
- **Decision Frequency**: Every #{timeRange} minutes (mid-to-low frequency trading)
...
1. **take_profit** (float): Exact price level to take profits
    - Should offer **at least 2:1 reward-to-risk ratio**
...
6. **effective_price_range** (object): Price range in which the strategy is valid and executable
    ...
    - When action is not "nothing|hold", you MUST provide an effective_price_range.
...
- ~10 recent data points per indicator (#{shortInterval} intervals)
- ~10 recent data points for #{longInterval} timeframe
