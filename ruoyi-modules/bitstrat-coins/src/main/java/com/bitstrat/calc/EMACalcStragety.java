...
@Service
public class EMACalcStragety implements MarketDataCalcStragety<List<Double>> {
    @Override
    public List<Double> calc(ExtConfigItem rule, BarSeries series, int size) {
        ...
        if (NumberUtils.isParsable(value)) {
            ...
            return MidPriceExtractor.getEmaValues(series,period,size);
        }else{
            throw new RuntimeException("EMA Config ERROR,Please Check your Config");
        }
    }
    ...
    public String getType() {
        return IndicatorType.EMA.getType();
    }
}
