...
@Service
public class SMACalcStragety implements MarketDataCalcStragety<List<Double>> {
    @Override
    public List<Double> calc(ExtConfigItem rule, BarSeries series, int size) {
        ...
        if (NumberUtils.isParsable(value)) {
            ...
            return MidPriceExtractor.getSmaValues(series,period,size);
        }else{
            throw new RuntimeException("SMA Config ERROR,Please Check your Config");
        }
    }
    @Override
    ...
    public String getType() {
        return IndicatorType.SMA.getType();
    }
}
