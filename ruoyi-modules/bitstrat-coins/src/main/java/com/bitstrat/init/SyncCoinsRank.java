...
import com.bitstrat.utils.CoinsRankUtils;
...
/**
 * 正向分数计算规则:
 * 1、计算24小时内比特币涨5个点。所有币，只要超过5，扣1分，
 *      如果跌5个点，山寨币跌少于5个点扣1分
 * 2、每一个小时统计，BTC
 *  BTC如果是涨，山寨币也是涨，但没涨过BTC，加1分，山寨币是跌的或者涨幅没超过BTC就不加分，
 *  BTC如果是跌，山寨币也是跌的，跌幅超过BTC，加1分，如果山寨币是涨的或者跌没超过BTC就不加分.
 */
@Slf4j
@Component
public class SyncCoinsRank {
    ...
    @Autowired
    ICoinsRankService iCoinsRankService;
...
//    @Scheduled(cron = "0 */5 * * * *")
    ...
    public synchronized void run() throws Exception {
        try{
            SyncStatusContext.start();
            this.runContent();
        }finally {
            SyncStatusContext.stop();
        }
    }
    public synchronized void runContent() throws Exception {
        log.info("开始同步计算山寨币相关表");
        ...
        if(!btcusdt.isEmpty()){
            ...
            for (BybitSymbolInfo bybitSymbolInfo : symbolsLiner) {
                ...
                if(!klinesData.isEmpty()){
                    ...
                    CoinsRankBo coinsRankBo = iCoinsRankService.selectBySymbol(bybitSymbolInfo.getSymbol());
                    if(Objects.nonNull(coinsRankBo)){
                        coinsRankBo.setMarketPrice(targetClose.toPlainString());
                        CoinsRankUtils.calcCoinsRankScore(coinsRankBo, score);
                        coinsRankBo.setPercentage(targetPercent.doubleValue());
                        coinsRankBo.setUpdateTime(new Date());
                        iCoinsRankService.updateByBo(coinsRankBo);
                    }else{
                        CoinsRankBo insertBo = new CoinsRankBo();
...
//                        CoinsRankUtils.calcCoinsRankScore(coinsRankBo, score);
                        ...
                        iCoinsRankService.insertByBo(insertBo);
                    }
...
//                        CoinsRankUtils.calcCoinsRankScore(coinsRankBo, score);
    ...
    /**
     * 检查并打分
     24小时 比特币涨5个点。所有币，只要超过5，不管多少分，都不上榜，
     如果跌5个点，山寨币跌少于5个点不上榜
     *
     * @param btcPercent
     * @param targetPercent
     ...
     * 如果是涨，山寨币也是涨，但没涨过BTC，加分，山寨币是跌的或者涨幅没超过BTC就不加分，
     * BTC如果 是跌，山寨币也是跌的，跌幅超过BTC，加1分，如果山寨币是涨的或者跌没超过BTC就不加分，然后按分数高低排名
     *
     * @param btcPercent
     * @param targetPercent
     * @return
    ...
    private int checkScore(BigDecimal btcPercent, BigDecimal targetPercent) {
        ...
        //BTC 是跌
        if(btcPercent.compareTo(BigDecimal.ZERO) < 0) {
            //山寨币也是跌的，跌幅超过BTC，加1分，如果山寨币是涨的或者跌没超过BTC就不加分，然后按分数高低排名
            if(targetPercent.compareTo(BigDecimal.ZERO) < 0 && targetPercent.compareTo(btcPercent) > 0) {
                return 1;
            }
    ...
    public void processRank() {
        //处理排名
        List<CoinsRankVo> coinsRankVos = iCoinsRankService.queryList(new CoinsRankBo());
        coinsRankVos.sort(Comparator.comparingDouble(CoinsRankVo::getScore).reversed());
        for (int i = 0; i < coinsRankVos.size(); i++) {
            CoinsRankVo coinsRankVo = coinsRankVos.get(i);
            coinsRankVo.setRank(i + 1);
            CoinsRankBo convert = MapstructUtils.convert(coinsRankVo, CoinsRankBo.class);
            iCoinsRankService.updateByBo(convert);
        }
    }
