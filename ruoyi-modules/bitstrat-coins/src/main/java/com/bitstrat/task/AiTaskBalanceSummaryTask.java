...
@Component
@Slf4j
@JobExecutor(name = "aiTaskBalanceSummaryTask")
public class AiTaskBalanceSummaryTask {
    ...
    public ExecuteResult jobExecute(JobArgs jobArgs) {
        SnailJobLog.LOCAL.info("开始定时统计AI任务余额");
        long startTimeStamp = System.currentTimeMillis();
        this.run(null);
        SnailJobLog.LOCAL.info("定时统计AI任务余额 耗时 {}ms", System.currentTimeMillis() - startTimeStamp);
        return ExecuteResult.success("定时统计AI任务余额成功");
    }
    ...
    public void run(List<CoinsAiTaskVo> coinsAiTaskVos) {
        ...
        for (CoinsAiTaskVo coinsAiTaskVo : coinsAiTaskVos) {
            ...
            if (Objects.nonNull(exchangeType)) {
                ...
                coinAITaskBalanceBo.setTaskId(coinsAiTaskVo.getId());
                coinAITaskBalanceBo.setCreateBy(coinsAiTaskVo.getCreateUserId());
                coinAITaskBalanceBo.setCreateTime(new Date());
                coinAITaskBalanceService.insertByBo(coinAITaskBalanceBo);
            }
        }
    }
    ...
    public ExecuteResult rootMapExecute(MapArgs mapArgs, MapHandler mapHandler) {
        SnailJobLog.LOCAL.info("定时统计AI任务余额任务开始切片 节点ID:{}", clusterManager.getNodeId());
        // 此处可以进行自定义的分片逻辑
        ...
        SnailJobLog.LOCAL.info("定时统计AI任务余额任务切片 SIZE={} 节点ID:{}",bsize, clusterManager.getNodeId());
        ...
        ExecuteResult secondMap = mapHandler.doMap(partition, "aiTaskBalanceSummaryTaskSecondMap");
        SnailJobLog.LOCAL.info("定时统计AI任务余额任务切片结束 节点ID:{}", clusterManager.getNodeId());
        return secondMap;
    }

    // 由ROOT_MAP分片分发到SECOND_MAP任务
    @MapExecutor(taskName = "aiTaskBalanceSummaryTaskSecondMap")
    public ExecuteResult secondMapExecute(MapArgs mapArgs) {
        SnailJobLog.REMOTE.info("定时统计AI任务余额任务执行切片 {} 节点ID {}", JSON.toJSONString(mapArgs.getMapResult()),clusterManager.getNodeId());
